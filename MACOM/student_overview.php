<?php
// student_overview.php (Refactored and Enhanced)
require_once 'db_config.php'; // Connects to DB, starts session
// require_once 'auth.php';  // If you have common functions like get_session_value, include it
                        // For now, I'll define get_session_value locally if not found.
                        ...
                        <div class="student-card-item" data-search-payload="<?php echo strtolower(htmlspecialchars($student['full_name'] . ' ' . $student['admission_number'] . ' ' . $student['class_name'] . ' ' . $student['nin_number'] . ' ' . $student['lin_number'])); ?>">
                            <div class="card h-100 student-card">
                                ...
                                <div class="card-footer d-flex justify-content-around align-items-center py-2">
                                    <button class="btn btn-sm btn-outline-primary flex-fill me-1" onclick="prepareEditModal(<?php echo $student['id']; ?>)">
                                    ...
                                    <button class="btn btn-sm btn-outline-danger flex-fill ms-1" onclick="confirmDeleteStudentDialog(<?php echo $student['id']; ?>, '<?php echo htmlspecialchars(addslashes($student['full_name'])); ?>')">
                        ...
                        <tbody id="studentsDisplayTableBody">
                            <?php if (!empty($students_display_list)): ?>
                                ...
                                <tr data-search-payload-table="<?php echo strtolower(htmlspecialchars($student['full_name'] . ' ' . $student['admission_number'] . ' ' . $student['class_name'] . ' ' . $student['nin_number'] . ' ' . $student['lin_number'])); ?>">
                                    ...
                                    <td>
                                        <button class="btn btn-sm btn-outline-primary" onclick="prepareEditModal(<?php echo $student['id']; ?>)"><i class="fas fa-edit"></i></button>
                                        <a href="student_detials.php?id=<?php echo $student['id']; ?>" class="btn btn-sm btn-outline-info"><i class="fas fa-eye"></i></a>
                                        <button class="btn btn-sm btn-outline-danger" onclick="confirmDeleteStudentDialog(<?php echo $student['id']; ?>, '<?php echo htmlspecialchars(addslashes($student['full_name'])); ?>')"><i class="fas fa-trash"></i></button>
                                    </td>
                                </tr>
                                <?php endforeach; ?>
                            <?php else: ?>
                                ...
                                <div class="row g-3 mb-3">
                                    ...
                                    <div class="col-md-4"><label for="s_sex" class="form-label">Sex <span class="text-danger">*</span></label>
                                        <select class="form-select" id="s_sex" name="sex" required>
                                            ...
                                            <option value="<?php echo $option; ?>" <?php echo (isset($form_data_session['sex']) && $form_data_session['sex'] == $option) ? 'selected' : ''; ?>><?php echo $option; ?></option>
                                            <?php endforeach; ?>
                                        </select>
                                    </div>
