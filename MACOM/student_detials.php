...
function get_current_academic_year_info($conn) { // Changed to get_current_academic_year_info
    if (!$conn) return null;
...
// --- AJAX ACTION HANDLING (Same as before) ---
...
if ($requested_action) {
    ...
    switch ($requested_action) {
        case 'get_available_optional_subjects_for_student':
            ...
            $class_level_ajax = filter_input(INPUT_POST, 'class_level', FILTER_SANITIZE_STRING);
            ...
            if ($stmt_taken_ajax) {
                $stmt_taken_ajax->bind_param("ii", $student_id_ajax, $academic_year_id_ajax); $stmt_taken_ajax->execute();
                $result_taken_ajax = $stmt_taken_ajax->get_result();
                while ($row_taken_ajax = $result_taken_ajax->fetch_assoc()) { $taken_subjects_ids_ajax[] = $row_taken_ajax['subject_id']; }
                $stmt_taken_ajax->close();
            }
            $available_subjects_ajax = [];
            $sql_available_ajax = "SELECT id, subject_name, subject_code, level_offered FROM subjects WHERE is_optional = 1";
            $params_ajax = []; $types_ajax = "";
            if (!empty($class_level_ajax) && ($class_level_ajax === 'O-Level' || $class_level_ajax === 'A-Level')) {
                $sql_available_ajax .= " AND (level_offered = ? OR level_offered = 'Both')";
                $params_ajax[] = $class_level_ajax; $types_ajax .= "s";
            }
            $sql_available_ajax .= " ORDER BY subject_name";
            $stmt_available_ajax = $conn->prepare($sql_available_ajax);
            if ($stmt_available_ajax) {
                if (!empty($params_ajax)) $stmt_available_ajax->bind_param($types_ajax, ...$params_ajax);
                $stmt_available_ajax->execute(); $result_available_ajax = $stmt_available_ajax->get_result();
                while ($subject_ajax = $result_available_ajax->fetch_assoc()) {
                    $subject_ajax['is_taken'] = in_array($subject_ajax['id'], $taken_subjects_ids_ajax);
                    $available_subjects_ajax[] = $subject_ajax;
                }
                $stmt_available_ajax->close();
                $ajax_response = ['success' => true, 'subjects' => $available_subjects_ajax];
            } else { $ajax_response['message'] = 'Error preparing statement for available subjects: ' . $conn->error; }
            break;
...
if ($conn && $current_academic_year_id) {
    ...
    else error_log("SQL optional subjects: " . $conn->error);
}
...
if ($conn && $student['current_class_id']) {
    $stmt_compulsory = $conn->prepare("SELECT sub.id, sub.subject_name, sub.subject_code FROM class_subjects cs JOIN subjects sub ON cs.subject_id = sub.id WHERE cs.class_id = ? AND sub.is_optional = 0 ORDER BY sub.subject_name");
     ...
     else error_log("SQL compulsory subjects: " . $conn->error);
}
$enrollment_history = [];
if ($conn) {
    ...
    else error_log("SQL enrollment history: " . $conn->error);
}
$student_age = calculate_age($student['date_of_birth']);
$default_photo = 'https://ui-avatars.com/api/?name='.urlencode($student['full_name']).'&background=0D6EFD&color=fff&size=180&font-size=0.33';
$student_photo = (!empty($student['photo']) && file_exists($student['photo'])) ? htmlspecialchars($student['photo']) : $default_photo;
?>
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <link href="style/styles.css" rel="stylesheet"> <!-- Your global styles -->
    <style>
        /* Theme variables from your example */
        :root {
            --card-bg-dark: rgba(255, 255, 255, 0.05);
            --text-color-dark: #f8f9fa;
            --border-color-dark: #495057;
            --primary-accent: #0d6efd; /* Your primary blue */
        ...
        body {
            background-color: var(--bs-body-bg); /* Let Bootstrap handle this */
            color: var(--bs-body-color);
            transition: background-color 0.3s ease, color 0.3s ease;
        }
        /* Profile container (left column) styling - from your example */
        .profile-container {
        ...
        /* Main details panel (right column) styling */
        .student-details-panel {
            background-color: var(--bs-tertiary-bg);
            border-radius: 0.75rem;
            box-shadow: 0 0.125rem 0.25rem rgba(0,0,0,0.075);
            border: 1px solid var(--bs-border-color);
            /* border-left: 5px solid var(--primary-accent); */ /* This will be on tab-content or panes */
            padding: 0; /* Remove padding to allow tabs to fill */
            height: 100%;
        }
        .student-details-panel .nav-tabs {
            border-bottom: 1px solid var(--bs-border-color);
            padding: 0.5rem 1rem 0 1rem; /* Padding for tabs */
        }
        .student-details-panel .nav-tabs .nav-link {
            border: none;
        ...
        .student-details-panel .nav-tabs .nav-link.active,
        .student-details-panel .nav-tabs .nav-link:hover {
            color: var(--primary-accent);
            border-bottom-color: var(--primary-accent);
            background-color: transparent; /* No background for active tab link */
        }
        .student-details-panel .tab-content {
            padding: 1.5rem; /* Padding for content inside tabs */
            border-left: 5px solid var(--primary-accent); /* THE BLUE THING */
            border-radius: 0 0 0.70rem 0; /* Keep border radius, adjust for left border */
            min-height: 300px; /* Ensure a minimum height */
        }
        .student-details-panel .tab-content h5.section-title {
            font-size: 0.9rem;
            font-weight: 600;
            color: var(--bs-secondary-color);
        ...
        /* Cards for sections below the main profile */
        ...
        .details-section-card .card-header {
            background-color: var(--bs-secondary-bg); /* Slightly different for header */
            border-bottom: 1px solid var(--bs-border-color);
            font-size: 1.1rem; font-weight: 500; padding: .75rem 1.25rem;
            color: var(--primary-accent);
        }
        ...
        .style-subject-badge { /* Renamed to avoid conflict */
            background-color: var(--primary-accent) !important; color: white !important;
            border-radius: .375rem; padding: .5em .8em; font-size: .85em; margin: .25rem;
            display: inline-flex; align-items: center; transition: all 0.2s ease;
        }
        .style-subject-badge-optional { /* New class for optional subjects */
        ...
        @media (max-width: 991px) { /* lg breakpoint */
            .profile-container .d-flex { flex-direction: column; text-align: center; }
            .profile-pic { margin-right: 0 !important; margin-bottom: 1rem; }
        }
    </style>
</head>
<body>
        ...
        <div class="flex-grow-1 p-3 p-md-4">
            <div class="d-flex justify-content-between align-items-center mb-4">
                ...
                <div>
                    ...
                    <button class="btn btn-sm btn-outline-primary" onclick="toggleTheme()" id="themeToggleBtn"><i class="fas fa-moon"></i></button>
                </div>
            </div>

            <!-- Main Profile Section (Your Preferred Layout) -->
            <div class="row mb-4">
                <div class="col-lg-4 mb-4 mb-lg-0">
                    <div class="profile-container p-3 p-md-4 h-100">
                        <div class="d-flex flex-column align-items-center text-center">
                            <img src="<?php echo $student_photo; ?>" alt="Student Photo" class="profile-pic mb-3">
                    ...
                    <div class="student-details-panel">
                        <ul class="nav nav-tabs" id="studentInfoTabs" role="tablist">
                            <li class="nav-item" role="presentation">
                                <button class="nav-link active" id="s-personal-tab" data-bs-toggle="tab" data-bs-target="#s-personal-pane" type="button" role="tab" aria-controls="s-personal-pane" aria-selected="true"><i class="fas fa-user-circle me-1"></i>Personal</button>
                            </li>
                            <li class="nav-item" role="presentation">
                                <button class="nav-link" id="s-academic-tab" data-bs-toggle="tab" data-bs-target="#s-academic-pane" type="button" role="tab" aria-controls="s-academic-pane" aria-selected="false"><i class="fas fa-graduation-cap me-1"></i>Academic</button>
                            </li>
                            <li class="nav-item" role="presentation">
                                <button class="nav-link" id="s-guardian-tab" data-bs-toggle="tab" data-bs-target="#s-guardian-pane" type="button" role="tab" aria-controls="s-guardian-pane" aria-selected="false"><i class="fas fa-users me-1"></i>Guardian</button>
                            </li>
                            <li class="nav-item" role="presentation">
                                <button class="nav-link" id="s-health-tab" data-bs-toggle="tab" data-bs-target="#s-health-pane" type="button" role="tab" aria-controls="s-health-pane" aria-selected="false"><i class="fas fa-notes-medical me-1"></i>Health & Docs</button>
                            </li>
                        </ul>
                        <div class="tab-content" id="studentInfoTabsContent">
                            ...
                            <div class="tab-pane fade" id="s-academic-pane" role="tabpanel" aria-labelledby="s-academic-tab" tabindex="0">
                                ...
                                <div class="info-item"><span class="info-label">Enrollment Status:</span> <span class="info-value"><span class="badge text-bg-<?php echo $student['enrollment_status'] == 'active' ? 'success' : 'secondary'; ?>"><?php echo htmlspecialchars(ucfirst($student['enrollment_status'])); ?></span></span></div>
                            ...
                            <div class="tab-pane fade" id="s-health-pane" role="tabpanel" aria-labelledby="s-health-tab" tabindex="0">
                                ...
                                <div class="info-item"><span class="info-label">Student Photo:</span>
                                    <span class="info-value">
                                    <?php if (!empty($student['photo']) && file_exists($student['photo']) && $student['photo'] !== $default_photo ): ?>
            ...
            <div class="d-flex flex-wrap gap-2 mb-4 justify-content-center">
                <button class="btn btn-primary action-btn" onclick="openEditStudentModal(<?php echo $student_id; ?>)"><i class="fas fa-edit me-1"></i> Edit Profile</button>
                <?php if ($current_academic_year_id) : ?>
                <button class="btn btn-success action-btn" data-bs-toggle="modal" data-bs-target="#manageSubjectsModal"><i class="fas fa-plus me-1"></i> Manage Optional Subjects</button>
                <?php else: ?>
                <button class="btn btn-success action-btn" disabled title="Current academic year not set"><i class="fas fa-plus me-1"></i> Manage Optional Subjects</button>
                <?php endif; ?>
                <button class="btn btn-danger action-btn" onclick="confirmDeleteStudent(<?php echo $student_id; ?>, '<?php echo htmlspecialchars(addslashes($student['full_name'])); ?>')"><i class="fas fa-trash me-1"></i> Delete Student</button>
            </div>
    ...
    <div class="modal fade" id="manageSubjectsModal" tabindex="-1" aria-labelledby="manageSubjectsModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-lg modal-dialog-scrollable">
            <div class="modal-content">
                ...
                <div class="modal-body"><p class="small text-muted">Academic Year: <strong><?php echo htmlspecialchars($current_academic_year_name); ?></strong></p><div id="availableOptionalSubjectsCheckboxes"><div class="text-center py-4"><div class="spinner-border text-primary"></div><p class="mt-2">Loading...</p></div></div></div>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() { const savedTheme = localStorage.getItem('themePreferenceMACOM'); if (savedTheme) { setThemePreference(savedTheme); } else { setThemePreference(window.matchMedia('(prefers-color-scheme: dark)').matches ? 'dark' : 'light'); } const manageSubjectsModalEl = document.getElementById('manageSubjectsModal'); if (manageSubjectsModalEl) { manageSubjectsModalEl.addEventListener('show.bs.modal', loadAvailableOptionalSubjects); } });
        function loadAvailableOptionalSubjects() {
            if (!currentAcademicYearId) { $('#availableOptionalSubjectsCheckboxes').html('<div class="alert alert-warning">Current academic year not set. Cannot load subjects.</div>'); return; }
            $('#availableOptionalSubjectsCheckboxes').html('<div class="text-center py-4"><div class="spinner-border text-primary"></div><p class="mt-2">Loading...</p></div>');
            $.ajax({
                url: 'student_details.php', method: 'POST', data: { action: 'get_available_optional_subjects_for_student', student_id: studentId, academic_year_id: currentAcademicYearId, class_level: studentClassLevel }, dataType: 'json',
                success: function(response) {
                    $('#availableOptionalSubjectsCheckboxes').empty();
                    if (response.success && response.subjects.length > 0) {
                        response.subjects.forEach(subject => { $('#availableOptionalSubjectsCheckboxes').append(`<div class="form-check mb-2"><input class="form-check-input" type="checkbox" value="${subject.id}" id="subjectOpt_${subject.id}" ${subject.is_taken ? 'checked' : ''}><label class="form-check-label" for="subjectOpt_${subject.id}">${subject.subject_name} (${subject.subject_code || 'N/A'})</label></div>`); });
                    } else if (response.success && response.subjects.length === 0) { $('#availableOptionalSubjectsCheckboxes').html('<div class="alert alert-info mb-0">No optional subjects available for this student\'s level or all are already chosen.</div>');
                    } else { $('#availableOptionalSubjectsCheckboxes').html(`<div class="alert alert-danger mb-0">${response.message || 'Failed to load subjects.'}</div>`); }
                },
                error: function(jqXHR) { console.error("AJAX Error:", jqXHR.responseText); $('#availableOptionalSubjectsCheckboxes').html('<div class="alert alert-danger mb-0">Error loading subjects. Check console.</div>');}
            });
        }
        $('#saveStudentOptionalSubjectsBtn').click(function() {
            const btn = $(this); const selectedSubjectIds = []; $('#availableOptionalSubjectsCheckboxes input[type=checkbox]:checked').each(function() { selectedSubjectIds.push($(this).val()); });
            if (!currentAcademicYearId) { alert('Current academic year not set.'); return; }
            btn.prop('disabled', true).html('<span class="spinner-border spinner-border-sm me-1"></span>Saving...');
            $.ajax({
                url: 'student_details.php', method: 'POST', data: { action: 'save_student_optional_subjects', student_id: studentId, academic_year_id: currentAcademicYearId, subject_ids: selectedSubjectIds }, dataType: 'json',
                success: function(response) { if (response.success) { alert(response.message || 'Subjects updated successfully!'); $('#manageSubjectsModal').modal('hide'); updateDisplayedOptionalSubjects(); } else { alert(response.message || 'Failed to update subjects.'); }},
        ...
        function updateDisplayedOptionalSubjects() {
            ...
            $.ajax({
                url: 'student_details.php', method: 'POST', data: { action: 'get_student_current_optional_subjects_html', student_id: studentId, academic_year_id: currentAcademicYearId }, dataType: 'html',
        ...
        function confirmDeleteStudent(id, name) { if (confirm(`ARE YOU SURE you want to delete student: ${name} (ID: ${id})?\n\nThis action is IRREVERSIBLE and will delete related records.`)) { const form = document.createElement('form'); form.method = 'POST'; form.action = 'student_overview.php'; form.innerHTML = `<input type="hidden" name="action" value="delete_student"><input type="hidden" name="student_id" value="${id}">`; document.body.appendChild(form); form.submit();}}
        function openEditStudentModal(id) { window.location.href = `student_overview.php?edit_attempt=${id}`; }
    </script>
</body>
</html>