...
if ($_SERVER["REQUEST_METHOD"] == "POST") { 
    ...
    if (isset($_POST['add_edit_class_action'])) { 
        ...
        header("Location: class_management.php"); exit();
    }
    ...
    <script>
        ...
        document.getElementById('assignSubjectsModal').addEventListener('show.bs.modal', function(event) {
            ...
            if (classDataAll[className] && classDataAll[className].class_db_id == classId) {
                // Fetch specific assigned subjects for THIS classId (more reliable)
                 $.ajax({
                    url: 'class_management.php?action=get_class_details&class_id=' + classId, // AJAX to get fresh assigned subjects
        ...
        document.querySelectorAll('.view-students-btn').forEach(btn => {
            btn.addEventListener('click', function() {
                ...
                $.ajax({
                    url: 'class_management.php', type: 'GET', data: { action: 'get_students_in_class', class_id: classId, academic_year_id: currentAcademicYearIdJS }, dataType: 'json',
        ...
        $('#searchStudentBtn').click(function() {
            ...
            $.ajax({
                url: 'class_management.php', type: 'POST', data: { action: 'search_student_for_class_add', searchTerm: searchTerm }, dataType: 'json',
                success: function(response) {
        ...
        $('#confirmAddStudentToClassBtn').click(function() {
            ...
            $.ajax({
                url: 'class_management.php', type: 'POST', data: { action: 'add_student_to_class_enrollment', student_id: studentIdToAdd, class_id: targetClassId, academic_year_id: currentAcademicYearIdJS }, dataType: 'json',
                success: function(response) {
                    if (response.success) {
